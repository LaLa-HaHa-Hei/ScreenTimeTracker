...
const maxValue = computed(() => props.usage ? Math.max(...props.usage) : 0)
const midValue = computed(() => maxValue.value / 2)
const avgDuration = computed(() => {
    if (!props.usage) return 0
    const arr = props.usage
    if (!arr.length) return 0
    return arr.reduce((total: number, item: number) => total + item, 0) / arr.length
})
    ...
    tooltip: {
        ...
        formatter: (params: any) => {
            if (params && params.length > 0) {
                const item = params[0];
                return `${item.axisValue}~${parseInt(item.axisValue) + 1} 点<br/>${formatDuration(item.data)}`;
            }
            return '';
        }
    },
