...
import { getTotalHourlyUsage, getTotalDailyUsage } from '@/api'
...
async function loadTotalUsage() {
    if (props.mode === 'Hour') {
        const res = await getTotalHourlyUsage(
...
watch(
    () => [props.startDate, props.endDate, props.mode, props.excludedProcesses.join(',')],
    loadTotalUsage,
    { immediate: true },
)
