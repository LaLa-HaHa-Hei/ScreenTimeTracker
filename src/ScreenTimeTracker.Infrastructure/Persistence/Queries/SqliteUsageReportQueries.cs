...
public class SqliteUsageReportQueries(ScreenTimeDbContext dbContext) : IUsageReportReadService
{
    private readonly ScreenTimeDbContext _dbContext = dbContext;

    public async Task<IDictionary<DateOnly, long>> GetProcessDailyDistributionForPeriod(Guid processId, DateOnly startDate, DateOnly endDate, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<IDictionary<int, long>> GetProcessHourlyDistributionForDay(Guid processId, DateOnly date, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<IEnumerable<ProcessUsageRankEntry>> GetRankedProcessUsageForPeriod(
        DateOnly startDate, DateOnly endDate, int topN = 10, IEnumerable<Guid>? excludedProcessIds = null, CancellationToken cancellationToken = default)
    {
        var start = startDate.ToDateTime(TimeOnly.MinValue, DateTimeKind.Local);
        var end = endDate.ToDateTime(TimeOnly.MinValue, DateTimeKind.Local).AddDays(1);
    ...
    public async Task<IDictionary<DateOnly, long>> GetTotalDailyUsageForPeriod(DateOnly startDate, DateOnly endDate, IEnumerable<Guid>? excludedProcessIds = null, CancellationToken cancellationToken = default)
    {
        var start = startDate.ToDateTime(TimeOnly.MinValue, DateTimeKind.Local);
        var end = endDate.ToDateTime(TimeOnly.MinValue, DateTimeKind.Local).AddDays(1);
    ...
    public async Task<IDictionary<int, long>> GetTotalHourlyUsageForDay(DateOnly date, IEnumerable<Guid>? excludedProcessIds = null, CancellationToken cancellationToken = default)
    {
        var start = date.ToDateTime(TimeOnly.MinValue, DateTimeKind.Local);
        var end = start.AddDays(1);
