...
import { websiteQueries } from "../api/queries";
...
type Item = Pick<Website, "id" | "name" | "iconPath" | "iconPathLastUpdatedAt">;

export type WebsitePickerProps = {
  placeholder?: string;
} & (
  | {
      mode: "single";
...
export const WebsitePicker = (props: WebsitePickerProps) => {
  const { t } = useTranslation(["entity_website"]);
  let commonProps;
  ...
  const { data: websitesData, isLoading: isWebsitesDataLoading } = useQuery(
    websiteQueries.websites({ fields: "id,name,iconPath,iconPathLastUpdatedAt" }),
  ) as {
    data?: Item[];
    isLoading: boolean;
  };
  ...
  const selectedOptions = useMemo(() => {
    ...
    if (mode === "single") {
      const found = websitesData.find((item) => item.id === value);
      return found ? [found] : [];
    } else {
      const valueSet = new Set(value as string[]);
      return websitesData.filter((item) => valueSet.has(item.id));
    }
  }, [websitesData, value, mode]);

  // 数据校验：自动移除不存在的 ID
  useEffect(() => {
    if (!websitesData) return;
    const validIdSet = new Set(websitesData.map((e) => e.id));
  ...
  }, [websitesData, value, mode, onValueChange]);
  ...
  return (
    <Autocomplete
      {...autocompleteProps}
      multiple
      options={websitesData || []}
      loading={isWebsitesDataLoading}
      loadingText={t(($) => $.entity_website.ui.state.loading)}
        ...
        if (
          ...
          return (
            <Chip
              variant="outlined"
              size="small"
              label={t(($) => $.entity_website.ui.picker.selectedCount, { count: value.length })}
            />
          );
        else
      ...
      noOptionsText={t(($) => $.entity_website.ui.state.noOptions)}
    />
  );
};
