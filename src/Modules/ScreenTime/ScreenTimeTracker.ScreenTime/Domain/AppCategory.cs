...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider adding the 'required' modifier or declaring as nullable.
    ...
    public static AppCategory Import(
        string name,
        string color,
        string? iconPath,
        DateTime iconPathLastUpdatedAt
    ) => new(Guid.CreateVersion7(), name, color, iconPath, iconPathLastUpdatedAt, false);

    public static AppCategory CreateUncategorized() =>
        new(UncategorizedId, "Uncategorized", "#8E8E93", null, DateTime.MinValue, true);

    public void Update(
        OptionalValue<string> name = default,
        OptionalValue<string> color = default,
        OptionalValue<string?> iconPath = default,
        DateTime? iconPathLastUpdatedAt = null
    )
    {
