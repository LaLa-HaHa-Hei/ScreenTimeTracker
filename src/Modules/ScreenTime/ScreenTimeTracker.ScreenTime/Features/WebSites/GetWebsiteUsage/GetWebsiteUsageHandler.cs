...
public class GetWebsiteUsageHandler(
...
) : IRequestHandler<GetWebsiteUsageQuery, List<GetWebsiteUsageResponseItem>>
{
    ...
    )
    {
        ...
        if (activeSession is not null)
        {
            ...
            {
                var included = request.IncludedIds?.Contains(activeSession.WebsiteId) ?? true;
                var notExcluded =
                    request.ExcludedIds is null
                    || !request.ExcludedIds.Contains(activeSession.WebsiteId);
                var shouldInclude = request.IncludedIds is not null ? included : notExcluded;

                if (shouldInclude)
                    sessions.Add(
                        new { activeSession.StartTime, EndTime = activeSession.LastActiveAt }
                    );
            }
        }
        ...
        // 组装最终结果（保留到秒）
        var result = new List<GetWebsiteUsageResponseItem>(timeRanges.Count);
