...
public partial class WebsiteUsageSessionOptimizer(
            ...
            while (await timer.WaitForNextTickAsync(stoppingToken))
            {
                ...
                for (int i = 0; i < sessions.Count; i++)
                {
                    ...
                    for (int j = i + 1; j < sessions.Count; j++)
                    {
                        ...
                        if (
                            next.StartTime
                            > current.EndTime + settings.WebsiteTracking.UsageSessionMergeTolerance
                        )
                            break;
                ...
                foreach (var session in sessions)
                {
                    ...
                    // 如果时长小于最小有效时长则删除
                    if (duration < settings.WebsiteTracking.MinValidUsageSessionDuration)
                ...
                if (sessionsToRemove.Count != 0)
                    context.WebsiteUsageSessions.RemoveRange(sessionsToRemove);
