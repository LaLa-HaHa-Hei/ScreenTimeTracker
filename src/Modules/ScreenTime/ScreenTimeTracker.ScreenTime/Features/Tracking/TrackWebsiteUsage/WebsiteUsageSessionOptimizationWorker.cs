...
public partial class WebsiteUsageSessionOptimizationWorker(
...
) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        using var timer = new PeriodicTimer(
            settings.WebsiteTracking.UsageSessionOptimizationInterval
        );
        try
        {
            while (await timer.WaitForNextTickAsync(stoppingToken))
            {
                ...
                if (
                    latestSettings.WebsiteTracking.UsageSessionOptimizationInterval
                    != settings.WebsiteTracking.UsageSessionOptimizationInterval
                )
                    timer.Period = latestSettings.WebsiteTracking.UsageSessionOptimizationInterval;
                ...
                var firstUnoptimized = await context
                    .WebsiteUsageSessions.Where(s => !s.IsOptimized && s.EndTime <= cutoffTime)
                ...
                var result = WebsiteUsageSessionOptimizer.Optimize(
                    sessions,
                    settings.WebsiteTracking.UsageSessionMergeTolerance,
                    settings.WebsiteTracking.MinValidUsageSessionDuration
                );

                if (result.SessionsToRemove.Count != 0)
                    context.WebsiteUsageSessions.RemoveRange(result.SessionsToRemove);
