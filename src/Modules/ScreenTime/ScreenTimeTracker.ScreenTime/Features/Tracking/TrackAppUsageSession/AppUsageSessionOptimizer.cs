...
) : BackgroundService
{
    ...
    {
        LogAppUsageSessionOptimizerStarting(logger);

        var settings = await GetUserSettingsAsync(stoppingToken);
        using var timer = new PeriodicTimer(settings.AppUsageSessionOptimizationInterval);
        try
        {
            while (await timer.WaitForNextTickAsync(stoppingToken))
            {
                ...
                if (
                    latestSettings.AppUsageSessionOptimizationInterval
                    != settings.AppUsageSessionOptimizationInterval
                )
                    timer.Period = latestSettings.AppUsageSessionOptimizationInterval;
                ...
                for (int i = 0; i < sessions.Count; i++)
                {
                    ...
                    for (int j = i + 1; j < sessions.Count; j++)
                    {
                        ...
                        if (
                            next.StartTime
                            > current.EndTime + settings.AppUsageSessionMergeTolerance
                        )
                            break;
                ...
                foreach (var session in sessions)
                {
                    ...
                    // 如果时长小于最小有效时长则删除
                    if (duration < settings.MinValidAppUsageSessionDuration)
    ...
    [LoggerMessage(Level = LogLevel.Information, Message = "AppUsageSessionOptimizer is starting.")]
    private static partial void LogAppUsageSessionOptimizerStarting(ILogger logger);
