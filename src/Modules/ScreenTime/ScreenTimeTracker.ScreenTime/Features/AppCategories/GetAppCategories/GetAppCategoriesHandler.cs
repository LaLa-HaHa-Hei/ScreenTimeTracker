...
public class GetAppCategoriesHandler(ScreenTimeDbContext context)
    : IRequestHandler<GetAppCategoriesQuery, List<Dictionary<string, object?>>>
{
    ...
    )
    {
        ...
        {
            nameof(GetAppCategoriesResponseItem.IconPathLastUpdatedAt),
            [nameof(AppCategory.IconPathLastUpdatedAt)]
        },
        { nameof(GetAppCategoriesResponseItem.IsSystem), [nameof(AppCategory.IsSystem)] },
    };

    // 各字段的计算方式字典
    private static readonly Dictionary<string, Func<AppCategory, object?>> _fieldCalculations = new(
        StringComparer.OrdinalIgnoreCase
    )
    {
    ...
    private static Expression<Func<AppCategory, AppCategory>> BuildEntitySelector(
